...
class CogniField:
    ...
    def __init__(
        ...
        from cognifield.llm.base import create_llm_client
    ...
    def think(self, input_text: str) -> Dict:
        ...
        llm_text = ""
        from cognifield.llm.base import MockLLM as _MockLLM
        if self._llm.is_available() and not isinstance(self._llm, _MockLLM):
            ctx = {
        ...
        elif isinstance(self._llm, _MockLLM):
    ...
    def _synthesize(
        ...
        # Priority 1: exact subject+predicate match (e.g. apple.edible)
        for key, rec in gc_results.items():
            parts = key.split(".")
            if len(parts) == 2:
                subj, pred = parts
                if subj in input_lower and pred in input_lower:
                    target_belief = rec
                    break

        # Priority 2: subject + safety predicate (edible/toxic)
        if target_belief is None:
            for key, rec in gc_results.items():
                parts = key.split(".")
                if len(parts) == 2:
                    subj, pred = parts
                    if subj in input_lower and pred in ("edible","toxic","safe","fragile"):
                        target_belief = rec
                        break

        # Priority 3: subject match on any predicate — prefer edible
        if target_belief is None:
            candidates = []
            for key, rec in gc_results.items():
                parts = key.split(".")
                if len(parts) == 2:
                    ...
                    if subj in input_lower:
                        priority = 10 if pred == "edible" else 1
                        candidates.append((priority, rec))
            if candidates:
                target_belief = max(candidates, key=lambda x: x[0])[1]

        # Priority 4: most confident authoritative belief
        if target_belief is None and gc_results:
            auth = {k: v for k, v in gc_results.items() if v.is_authoritative}
            if auth:
                target_belief = max(auth.values(), key=lambda r: r.confidence)
            else:
                target_belief = max(gc_results.values(), key=lambda r: r.confidence, default=None)
        ...
        if target_belief is None:
            ...
            reasoning.append("Recommend gathering more observations before acting.")
        elif (target_belief.key.endswith(".category")
               or target_belief.key.endswith(".color")
               or target_belief.key.endswith(".size")):
            # Avoid using non-safety beliefs as decisions
            decision   = "insufficient_data"
            confidence = 0.30
            reasoning.append(f"Only structural knowledge available: {target_belief.key}={target_belief.value}")
            reasoning.append("No safety-relevant belief (edible/toxic) found — investigate first.")
